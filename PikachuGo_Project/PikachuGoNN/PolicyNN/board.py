...
class Go(object):
    ...
    # 判断比之前好多少
    def evaluate_2(self):
        overall_judge = np.zeros((19, 19), dtype=np.int16)
        for x in range(19):
            for y in range(19):
                arr = None
        ...
        for i in range(19):
            for j in range(19):
                if self.board[i][j] == BLACK_STONE:
                    ...
                    if overall_judge[i][j] > + 256:
                        cnt_black += 256
                        overall_judge[i][j] = 256
                    elif overall_judge[i][j] > 0:
                        cnt_black += overall_judge[i][j]
                    elif overall_judge[i][j] < - 256:
                        cnt_white += 256
                        overall_judge[i][j] = -256
                    elif overall_judge[i][j] < 0:
        ...
        go_plot.go_plot_plus3(overall_judge, self.board)
