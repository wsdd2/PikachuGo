...
path = os.listdir('E:/value_shuffle_2')
...
no = 0
for file in path:
    data = np.load('E:/value_shuffle_2/' + file)
    label = np.load('E:/value_shuffle_label_2/' + file)
    for i in range(300):
        if i*10000+10000 > data.shape[0]:
            np.save('E:/value_target_data/val' + str(no), data[i * 10000: min(i * 10000 + 10000, data.shape[0])])
            np.save('E:/value_target_label/val' + str(no), label[i * 10000: min(i * 10000 + 10000, label.shape[0])])
            data = None
            label = None
            break
        np.save('E:/value_target_data/'+str(no), data[i*10000: min(i*10000+10000, data.shape[0])])
        np.save('E:/value_target_label/'+str(no), label[i*10000: min(i*10000+10000, label.shape[0])])
        no += 1
...
for file in path:
    ...
    np.save('E:/c_npy/' + file, data)
    np.save('E:/c_label_npy/' + file, label)
...
"""
        ...
        aaa = compressed.reshape(-1, 1, 361)
        exp2 = exp2.T
        exp2 = exp2.reshape(1, 16, 1)
        recover = ((np.bitwise_and(aaa, exp2) > 0) + 0).reshape(-1, 16, 19, 19)
        print recover[0]
